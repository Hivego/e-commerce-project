package com.lianghao.streaming
...
object ConnHelper extends Serializable{
  // 懒变量定义,使用的时候才初始化
  lazy val jedis = new Jedis("recommend")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://recommend:27017/recommender"))
}
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 创建spark conf
    val sparkConf = new SparkConf().setMaster(config("spark.cores")).setAppName("SteamingRecommender")
    ...
    // 定义广播变量
    val simProdcutsMatrixBC = sc.broadcast(simProductsMatrix)

    // 创建kafka配置参数
    val kafaParam = Map(
      "bootstrap.servers" -> "recommend:9092",
    ...
    val kafkaStream = KafkaUtils.createDirectStream[String, String](ssc,
      ...
      ConsumerStrategies.Subscribe[String, String](Array(config("kafKa.topic")), kafaParam)
    )

    // 对kafkaSteam进行处理,产生评分流,userId|productId|score|timestamp
      ...
      rdds =>
        rdds.foreach {
          case (userId, productId, score, timestamp) =>
            ...
            val candidateProducts = getTopSimProducts(MAX_SIM_PRODUCTS_NUM, productId, userId, simProdcutsMatrixBC.value)
            // 3. 计算每个备选商品的推荐优先级,得到当前用户的实时推荐列表,保存成Array[(productId, score)]
            val streamRecs = computeProductScore(candidateProducts, userRecentlyRatings, simProdcutsMatrixBC.value)
            // 4. 把推荐列表保存到MongoDB
    ...
    def getUserRecentlyRatings(num: Int, userId: Int, jedis: Jedis): Array[(Int, Double)] = {
      //从redis中用户的评分队列里获取评分数据, list键名为uid:USERID,值格式是PRODUCTID:SCORE
    ...
    // 获取当前商品的相似度列表,并过滤掉用户已经评分过的,作为备选列表
    def getTopSimProducts(num: Int,
                          productId: Int,
                          userId: Int, simProducts: collection.Map[Int, Map[Int, Double]])
    ...
    : Array[(Int, Double)] = {
      ...
      // 遍历每个备选商品,计算和已评分商品间的相似度
      for (candidateProdect <- candidateProducts; userRecentlyRating <- userRecentlyRatings) {
        // 从相似度矩阵中获取当前备选商品和当前已评分商品间的相似度
        val simScore = getProductsSimScore(candidateProdect, userRecentlyRating._1, simProducts)
        if (simScore > 0.4) {
          // 按照公式进行加权计算,得到基础评分
          scores += ((candidateProdect, simScore * userRecentlyRating._2))
          if (userRecentlyRating._2 > 3) {
            increMap(candidateProdect) = increMap.getOrDefault(candidateProdect, 0) + 1
          } else {
            decrMap(candidateProdect) = increMap.getOrDefault(candidateProdect, 0) + 1
          }
        }
      }
      ...
      scores.groupBy(_._1).map {
        case (productId, scoreList) =>
          (productId, scoreList.map(_._2).sum / scoreList.length + log(increMap.getOrDefault(productId, 1)) - log(decrMap.getOrDefault(productId, 1)))
      }
        // 返回推荐列表,按照得分排序
        .toArray
        .sortWith(_._2 > _._2)
