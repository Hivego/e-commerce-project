...
// 定义商品相似度列表
case class ProductRece(productId: Int, recs: Seq[Recommendation])
...
object ItemCFRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 统计每个商品的评分个数,安装productId来做group by
    val productRatingCountDF = ratingDF.groupBy("productId").count()
    // 在原有的评分表上rating添加count
    val ratingWithCountDF = ratingDF.join(productRatingCountDF, "producntId")
    ...
    // 按照product1,product2做group by,统计userId的数量,就是对两个商品同时评分的人数
    ...
    val simDF = cooccurrenceDF.map{
      ...
      .map{
        case (productId, recs) =>
          ProductRece(productId, recs.toList
