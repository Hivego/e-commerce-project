...
object ContentRecommender {
  // 定义mongodb中存储的表名
  val MONGODB_PROUCT_COLLECTION = "Product"
  val CONTENT_PROUCT_RECS = "ContentBasedProductRecs"

  def main(args: Array[String]): Unit = {
    val config = Map(
      "spark.cores" -> "local[*]",
      "mongo.uri" -> "mongodb://recommend:27017",
    ...
    val productTagsDF = spark.read
      ...
      .option("collection", MONGODB_PROUCT_COLLECTION)
    ...
    productRecs.write
      .option("uri", mongoConfig.uri)
      .option("collection", CONTENT_PROUCT_RECS)
      .mode("overwrite")
      .format("com.mongodb.spark.sql")
      .save()
